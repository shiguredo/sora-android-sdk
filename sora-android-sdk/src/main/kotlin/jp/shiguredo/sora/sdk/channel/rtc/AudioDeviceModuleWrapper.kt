...
class AudioDeviceModuleWrapper(val adm: AudioDeviceModule) {
    companion object { private val TAG = AudioDeviceModuleWrapper::class.simpleName }
    ...
    fun pauseRecording(): Boolean {
        return try {
            val rec = findAudioRecordObject() ?: run {
    ...
    fun resumeRecording(): Boolean {
        return try {
            ...
            val state = savedRecordingState ?: run {
    ...
    private fun reinitializeRecording(state: RecordingState): Boolean {
        return try {
            ...
            val initMethod = rec.javaClass.declaredMethods.firstOrNull {
            ...
            val startMethod = rec.javaClass.declaredMethods.firstOrNull {
    ...
    private fun findAudioThread(rec: Any): Thread? {
        return try {
            rec.javaClass.declaredFields.firstOrNull { f ->
    ...
    private fun findAudioRecordObject(): Any? {
        return try {
            ...
            adm.javaClass.declaredFields.firstOrNull { it.name.equals("audioInput", true) }
                ?.let { it.isAccessible = true; return it.get(adm) }
            // 2) 型名に WebRtcAudioRecord を含むもの
            adm.javaClass.declaredFields.firstOrNull {
            ...
            adm.javaClass.declaredFields.forEach { f ->
                try {
                    ...
                    // ネスト1段探索
                    v.javaClass.declaredFields.firstOrNull { nf ->
    ...
    private fun findInnerAndroidAudioRecord(rec: Any): Any? {
        return try {
            ...
            rec.javaClass.declaredFields.firstOrNull { f ->
                f.type.name.contains("android.media.AudioRecord") || f.type.simpleName == "AudioRecord" || f.name.equals("audioRecord", true)
            }?.let { f ->
                f.isAccessible = true
                return f.get(rec)
            }
