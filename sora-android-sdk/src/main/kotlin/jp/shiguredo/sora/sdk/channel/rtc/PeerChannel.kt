...
class PeerChannelImpl(
    ...
    override fun setAudioHardwareMuted(muted: Boolean): Boolean = runBlocking {
        setAudioHardwareMutedAsync(muted)
    }

    private suspend fun setAudioHardwareMutedAsync(muted: Boolean): Boolean = withContext(executor.asCoroutineDispatcher()) {
        if (muted) {
    ...
    private fun muteAudioHardware() {
        ...
        try { audioSender?.setTrack(null, false) } catch (_: Exception) {}
        try { localAudioManager.dispose() } catch (_: Exception) {}
