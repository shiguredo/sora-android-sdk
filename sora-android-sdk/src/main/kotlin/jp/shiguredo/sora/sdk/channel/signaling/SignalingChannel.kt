...
class SignalingChannelImpl @JvmOverloads constructor(
    ...
    companion object {
        ...
        private const val DISCONNECT_TIMEOUT_MS = 5000L // 5秒のタイムアウト
    }
    ...
    private var disconnectResult: SignalingDisconnectResult? = null
            ...
            return runBlocking(Dispatchers.IO) {
                try {
                    ...
                    withTimeout(DISCONNECT_TIMEOUT_MS) {
                        ...
                        // onClosed内で設定されたdisconnectResultを取得
                        val result = disconnectResult
                        if (result != null) {
                            SoraLogger.i(TAG, "[signaling:$role] WebSocket disconnected: code=${result.code}, reason=[${result.reason}]")
                        } else {
                            SoraLogger.w(TAG, "[signaling:$role] WebSocket disconnected but no result available")
                        }
                        result
                    }
