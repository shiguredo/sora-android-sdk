...
- `src/androidTest/AndroidManifest.xml` を新規作成する。カメラ・マイク権限の宣言は不要だが、以下の権限が必要:
  - `android.permission.INTERNET` — WebSocket 接続に必須
  - `android.permission.ACCESS_NETWORK_STATE` — `PeerConnectionFactory.initialize()` が内部でネットワーク状態を参照するため必須。不足時は WebRTC JNI abort が発生する
...
- **音声ダミー入力の実現** — shiguredo の libwebrtc AAR に `kDummyAudio` の Java ラッパーがないため、`webrtc-build` へのパッチ追加と AAR 再ビルドが必要。issue 0059 で対応
...
`src/androidTest/kotlin/jp/shiguredo/sora/sdk/DummyVideoCapturer.kt` に `VideoCapturer` 実装を追加した。7 色（白/黄/シアン/緑/マゼンタ/赤/青）の横カラーバーを I420 フレームとして生成し、フレームごとに 4px 横シフトする。

- フレーム生成: `JavaI420Buffer.allocate(width, height)` + Y/U/V 各プレーンをストライド考慮で埋める
- スレッド管理: `SurfaceTextureHelper.create("CaptureThread", eglContext)` の `Handler` を借用（自前 HandlerThread 不要）
- タイムスタンプ: `TimestampAligner().translateTimestamp(TimeUnit.MILLISECONDS.toNanos(System.currentTimeMillis()))`
- `dispose()` 責務: `handler.removeCallbacks(null)` による post 済みタスク停止のみ。`SurfaceTextureHelper.dispose()` は `RTCLocalVideoManager.dispose()` が担当
- 二重 `dispose()` 対策: `AtomicBoolean isDisposed` でガード

#### build.gradle.kts / libs.versions.toml

- `testInstrumentationRunner = "androidx.test.runner.AndroidJUnitRunner"` を追加
- `androidTest` sourceSet を追加 (`src/androidTest/kotlin`)
- `androidTestImplementation` に `junit`, `androidx-test-core`, `androidx-test-runner`, `androidx-test-ext-junit` を追加
- `BuildConfig.TEST_SIGNALING_URL` を環境変数 `SORA_SIGNALING_URL` から埋め込み
- Version Catalog に `androidx-test-runner = "1.6.2"`, `androidx-test-ext-junit = "1.2.1"` を追加

#### AndroidManifest.xml

`src/androidTest/AndroidManifest.xml` を作成。以下の権限が必要（不足時は WebSocket 接続失敗や JNI abort が発生）:

- `android.permission.INTERNET` — WebSocket 接続
- `android.permission.ACCESS_NETWORK_STATE` — `PeerConnectionFactory.initialize()` が内部で参照

#### SoraE2ETest

`src/androidTest/kotlin/jp/shiguredo/sora/sdk/SoraE2ETest.kt` を作成。映像送信確認テスト 1 件:

1. **映像が送信されること**: `enableVideoUpstream(dummyCapturer, null)` + 音声 upstream なしの最小構成。接続成功後、`capturer.startCapture(640, 480, 30)` を手動呼び出し。フレーム生成確認後、`getStats()` の outbound-rtp から `kind == "video"` の `bytesSent > 0 && packetsSent > 0` を最大 10 秒ポーリングで確認。

### 実装過程で判明した知見

| 知見 | 詳細 |
|---|---|
| `isOwnedCapturer` は常に `false` | `RTCComponentFactory.createVideoManager()` はユーザー提供 `VideoCapturer` に対して `RTCLocalVideoManager(capturer, cameraConfig)` を既定値 `isOwnedCapturer=false` で生成する。`cameraConfig` の有無にかかわらず SDK が `startCapture()` を呼ぶことはないため、テスト側で手動呼び出しが必要 |
| outbound-rtp のフィルタリング | stats には音声と映像の outbound-rtp が混在する。`kind` または `mediaType` が `"video"` のものだけを判定対象にする必要がある |
| `bytesSent`/`packetsSent` の型 | `Long` に直接キャストすると失敗する場合がある。`Number` 経由で `toLong()` するのが安全 |
| `ACCESS_NETWORK_STATE` 必須 | 不足時は `PeerConnectionFactory.initialize()` で JNI abort が発生する |
| 音声 upstream なしで映像テスト | `enableAudioUpstream()` + `initialAudioHardMute=true` でも `AUDIO_RECORD_INIT_ERROR` が発生するため、音声 upstream を完全に外す構成が最も安定する |
| `videoFrame.release()` 必須 | `onFrameCaptured()` 呼び出し後に `videoFrame.release()` しないと、`JavaI420Buffer.allocate()` で確保したネイティブバッファが回収されずリークする。`VideoSource` 側で必要な場合は内部で `retain()` されるため、即時 `release()` が正しい

### 未完了項目

- **音声ダミー入力** — `kDummyAudio` の Java ラッパーが shiguredo AAR に存在しないため、`webrtc-build` へのパッチ追加と AAR 再ビルドが必要。別 issue で対応

### 変更履歴

`CHANGES.md` の `### misc` にエントリを追加した。
