...
本 issue はバグではない。Sora の設計思想として「一部でも不意に閉じたなら、それはもはや健全な接続ではないため、速やかに全体を閉じるべきである」という方針があり、`dataChannelSignalingCloseEvent` が null の場合に label を問わず `internalDisconnect(null)` を呼ぶ現在の実装はこの設計思想に沿った意図的な動作である。修正不要のため close とする。
